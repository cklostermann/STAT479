...
data{
  ...
  real<lower = 0> views[n]; // total number of views
...
transformed parameters{
  real<lower = 0> mean_likes;
  mean_likes = exp(likes[n]);
}
model{
  ...
  for(i in 1:n){
    popularity[i] ~ poisson(views[i]*mean_likes);
  }
}

generated quantities{
  vector<lower = 0, upper = 1>[n_grid] prob_grid; // likes/popularity for each combination of variables
  int<lower = 0> popularity_post; // posterior predictive draw
  vector<lower = 0>[n] post; // posterior predictive draws
  ...
  for(i in 1:n_grid){
    popularity_post = poisson_rng(views[i]*mean_likes); // posterior predictive draw
    post[i] = popularity_post;
  }
}
